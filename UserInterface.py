...
from ObjectTracker import track_object
...
class UserInterface:
    ...
    def draw():
        ...
        Label(root, text="Input and Output Settings", relief=SUNKEN, pady=3).grid(row=0, column=0, columnspan=5,
                                                                                  sticky=W + E)
        ...
        var_tracking_cb = BooleanVar(value=True)
        ...
        def start_video_processing():
            if rbutton_file.get() and not root.filename:
                messagebox.showwarning("Missing parameter", "No video selected", icon="warning")
                return
            if vid_out_var.get() and not root.videofiledir:
                messagebox.showwarning("Missing parameter", "No video output selected", icon="warning")
                return
            if text_out_var.get() and not root.textfiledir:
                messagebox.showwarning("Missing parameter", "No text output selected", icon="warning")
                return
            if not validate_threshold(entry_face_det_prob.get()):
                messagebox.showwarning("Bad value", "Passenger threshold should be between 0 and 1.", icon="warning")
                return
            if not validate_threshold(entry_vehicle_det_prob.get()):
                messagebox.showwarning("Bad value", "Vehicle threshold should be between 0 and 1.", icon="warning")
                return
            ...
            print("Video path: {} | Vid/camera: {} | Video output: {} {} | Text output: {} {}|"
                  ...
                  " Color: {} {} | Vehicle threshold: {}".format(root.filename, rbutton_file.get(), root.videofiledir,
                                                                 vid_out_var.get(), root.textfiledir,
                                                                 text_out_var.get(),
            ...
            track_object(None, video_path=root.filename if rbutton_file.get() else None,
                         vid_output_path=root.videofiledir if vid_out_var.get() else None,
                         text_output_path=root.textfiledir if text_out_var.get() else None,
                         tracking=var_tracking_cb.get(),
        ...
        Button(root, text="Detect", borderwidth=3, command=start_video_processing).grid(row=10, column=0, columnspan=5,
                                                                                        sticky=W + E)
