...
def draw_bbox(image, bboxes, CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence = True, Text_colors=(0,0,0), rectangle_colors='', tracking=False, color = True):
    ...
    if color:
        ...
        for i, bbox in enumerate(bboxes):
            ...
            color_dict.append(bincount_numexpr_app(cropped_object))
            #object_color = bincount_app(cropped_object)
            #score_str += " {}".format(object_color)
        #cv2.rectangle(image, (x1, y1), (x2, y2), bbox_color, bbox_thick)

    for i, bbox in enumerate(bboxes):
        coor = np.array(bbox[:4], dtype=np.int32)
        score = bbox[4] if not tracking else bbox[6]
        ...
        cv2.rectangle(image, (x1, y1), (x2, y2), bbox_color, bbox_thick) #bbox_thick*2
